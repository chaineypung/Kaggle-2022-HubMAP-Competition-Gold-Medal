...
class ParallelBlock(nn.Module):
    ...
    def forward_old(self, x1, x2, x3, x4, sizes):
...
class Coat_parallel_small_plus1(CoaT):
    def __init__(self, **kwargs):
        super(Coat_parallel_small_plus1, self).__init__(
            ...
            pretrain='../pretrain_wts/coat_small_7479cf9b.pth',
            **kwargs)
...
def criterion_aux_loss(logit, mask):
    # print('logit',logit.shape[-2:],mask.unsqueeze(1).shape)
    mask = F.interpolate(mask.unsqueeze(1), size=logit.shape[-2:], mode='nearest')
...
class CoaTNet(nn.Module):
    ...
    def load_pretrain(self, ):
        checkpoint = "/root/.cache/torch/hub/checkpoints/coat_small_7479cf9b.pth"
        checkpoint = torch.load(checkpoint, map_location=lambda storage, loc: storage)
        state_dict = checkpoint
        self.encoder.load_state_dict(state_dict['model'], strict=False)
        print(self.encoder.load_state_dict(state_dict['model'], strict=False))
    ...
    def __init__(self):
        ...
        self.decoder = daformer_conv3x3(encoder_dim=[152, 320, 320, 320, 320])
        self.aux = nn.ModuleList([
            nn.Conv2d(320, 1, kernel_size=1, padding=0) for i in range(4)
        ])
        self.fc = nn.Conv2d(256, 1, kernel_size=1, padding=0)
    ...
    def forward(self, batch):
        ...
        logit_fused, ind_logits = self.decoder(logit)
        logit_fused = self.fc(logit_fused)
        logit_fused = F.interpolate(logit_fused, size=None, scale_factor=4, mode='bilinear', align_corners=False)

        output = {}
        if 'loss' in self.output_type:
            output['bce_loss'] = F.binary_cross_entropy_with_logits(logit_fused, batch['mask'])
            for i in range(4):
                output['aux%d_loss' % i] = output['bce_loss']

        if 'inference' in self.output_type:
            output['valid_loss'] = F.binary_cross_entropy_with_logits(logit_fused, batch['mask'])
            output['probability'] = torch.sigmoid(logit_fused)
