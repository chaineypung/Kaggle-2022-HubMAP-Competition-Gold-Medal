...
def generate_new_sample(image_a, image_b, label_a, label_b):
    ...
    label_b = np.squeeze(label_b)
    spacing = 1
    label = np.copy(label_b)
    dis_array = get_distance(label, spacing)
    c = np.random.beta(1, 1)
    c = (c - 0.5) * 2
    if c > 0:
        lam = c * np.min(dis_array) / 2
    else:
        lam = c * np.min(dis_array)
    mask: object = (dis_array < lam).astype('float32')

    new_target_0 = np.expand_dims(image_a[0, :, :] * (mask == 0) + image_b[0, :, :] * mask, axis=0)
    new_target_1 = np.expand_dims(image_a[1, :, :] * (mask == 0) + image_b[1, :, :] * mask, axis=0)
    new_target_2 = np.expand_dims(image_a[2, :, :] * (mask == 0) + image_b[2, :, :] * mask, axis=0)
    new_target = np.concatenate((new_target_0, new_target_1, new_target_2), axis=0)
    new_target = torch.from_numpy(new_target)

    new_label = label_a * (mask == 0) + label_b * mask
    new_label = np.expand_dims(new_label, axis=0)
...
def get_fold_filelist(csv_file, K=3, fold=1, random_state=2020, validation=False, validation_r = 0.2):
    ...
    if validation is False: # 不设置验证集，则直接返回
        ...
        print(f"num of train: {len(train_set)}")
