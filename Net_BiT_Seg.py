...
class Net(nn.Module):
    ...
    def forward(self, batch):
        ...
        if 'loss' in self.output_type:
            if 0:
                edge_weight = 6.
                clsy = batch['cls'].cpu().numpy()
                batch['edge'][batch['edge'] == 0] = 1.
                batch['edge'][batch['edge'] == 1] = edge_weight
                if 2 in clsy:
                    index = np.argwhere(clsy != 2)
                    batch['edge'][index, :, :, :] = 1
                    output['bce_loss'] = F.binary_cross_entropy_with_logits(logit, batch['mask'], reduction='none')
                    output['bce_loss'] *= batch['edge']
                else:
                    output['bce_loss'] = F.binary_cross_entropy_with_logits(logit, batch['mask'], reduction='none')
            if 1:
            ...
            if 0:
                loss = F.binary_cross_entropy_with_logits(logit, batch['mask'], reduction='none')
                weight = torch.ones_like(loss)
                metric = -loss.detach().reshape((loss.shape[0], loss.shape[1] * loss.shape[2] * loss.shape[3]))
                weight = F.softmax(metric, 1)
                weight = weight / weight.mean(1).reshape((-1, 1))
                weight = weight.reshape((loss.shape[0], loss.shape[1], loss.shape[2], loss.shape[3]))
                for i in range(batch['mask'].shape[0]):
                    tag = set(batch['mask'][i].reshape(batch['mask'].shape[1] * batch['mask'].shape[2] * batch['mask'].shape[3]).tolist())
                    if len(tag) <= 1:
                        weight[i] = 1
                weight = weight.float()
                loss = loss * weight
